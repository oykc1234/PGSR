...
def training(dataset, opt, pipe, testing_iterations, saving_iterations, checkpoint_iterations, checkpoint, debug_from,
             depth_weight=0.1, depth_weight_from_iter=1000, depth_confidence_dir="depth_confidence_texture", depth_conf_min=0.05):
    first_iter = 0
    ...
    # ===== Preload depth and confidence maps =====
    depth_maps = {}
    conf_maps = {}
    depths_dir = os.path.join(dataset.source_path, "depths")
    conf_dir = os.path.join(dataset.source_path, depth_confidence_dir)
    if os.path.isdir(depths_dir) and os.path.isdir(conf_dir):
        print(f"[DepthConf] Loading depth maps from: {depths_dir}")
        print(f"[DepthConf] Loading confidence maps from: {conf_dir}")
        for fname in sorted(os.listdir(depths_dir)):
            if fname.endswith('.npy'):
                stem = os.path.splitext(fname)[0]
                depth_maps[stem] = os.path.join(depths_dir, fname)
        for fname in sorted(os.listdir(conf_dir)):
            if fname.endswith('.npy'):
                stem = os.path.splitext(fname)[0]
                conf_maps[stem] = os.path.join(conf_dir, fname)
        print(f"[DepthConf] Found {len(depth_maps)} depth maps, {len(conf_maps)} confidence maps")
    else:
        print(f"[DepthConf] WARNING: depth/confidence dirs not found, depth loss disabled")
        depth_weight = 0.0
    # ===== End preload =====
    ...
    normal_loss, geo_loss, ncc_loss, depth_loss = None, None, None, None
    ...
    for iteration in range(first_iter, opt.iterations + 1):
        ...
        need_plane = (iteration > opt.single_view_weight_from_iter) or (depth_weight > 0 and iteration > depth_weight_from_iter)
        ...
        depth_loss = None
        if depth_weight > 0 and iteration > depth_weight_from_iter and 'plane_depth' in render_pkg:
            cam_stem = os.path.splitext(viewpoint_cam.image_name)[0]
            if cam_stem in depth_maps and cam_stem in conf_maps:
                # Load mono depth and confidence
                mono_depth_np = np.load(depth_maps[cam_stem])
                conf_np = np.load(conf_maps[cam_stem]).copy()

                # Apply confidence threshold
                conf_np[conf_np < depth_conf_min] = 0.0

                # Convert to tensors
                mono_depth_t = torch.from_numpy(mono_depth_np).float().cuda()
                conf_t = torch.from_numpy(conf_np).float().cuda()

                # Get rendered depth
                rendered_depth = render_pkg['plane_depth'].squeeze()

                # Resize if needed
                rH, rW = rendered_depth.shape
                if mono_depth_t.shape[0] != rH or mono_depth_t.shape[1] != rW:
                    mono_depth_t = F.interpolate(mono_depth_t[None, None], size=(rH, rW),
                                                 mode='bilinear', align_corners=False).squeeze()
                    conf_t = F.interpolate(conf_t[None, None], size=(rH, rW),
                                           mode='bilinear', align_corners=False).squeeze()

                # Normalize rendered depth to 0-1 (per-frame, scale-invariant)
                rd_min = rendered_depth.min()
                rd_max = rendered_depth.max()
                rendered_depth_norm = (rendered_depth - rd_min) / (rd_max - rd_min + 1e-8)

                # Confidence-weighted L1 depth loss
                depth_diff = (rendered_depth_norm - mono_depth_t).abs()
                depth_loss = depth_weight * (conf_t * depth_diff).mean()
                loss += depth_loss
        # ===== End depth loss =====
...
if __name__ == "__main__":
    ...
    # ===== Depth confidence 新增参数 =====
    parser.add_argument("--depth_weight", type=float, default=0.1,
                        help="Weight for confidence-weighted depth loss")
    parser.add_argument("--depth_weight_from_iter", type=int, default=1000,
                        help="Start applying depth loss after this iteration")
    parser.add_argument("--depth_confidence_dir", type=str, default="depth_confidence_texture",
                        help="Subdirectory name for confidence .npy maps")
    parser.add_argument("--depth_conf_min", type=float, default=0.05,
                        help="Minimum confidence threshold (below this -> weight=0)")
    # ===== End =====
    ...
    training(lp.extract(args), op.extract(args), pp.extract(args), args.test_iterations,
             ...
             depth_weight_from_iter=args.depth_weight_from_iter,
