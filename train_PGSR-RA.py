...
def training(dataset, opt, pipe, testing_iterations, saving_iterations, checkpoint_iterations, checkpoint, debug_from):
    ...
    for iteration in range(first_iter, opt.iterations + 1):
        ...
        # single-view loss (IMPROVED: residual-aware adaptive weighting)
        if iteration > opt.single_view_weight_from_iter:
            ...
            # ====== Residual-Aware Adaptive Normal Loss ======
            # Compute per-pixel rendering residual as confidence signal.
            # High residual -> geometry constraint may hinder appearance -> reduce weight locally.
            # Low residual -> rendering is good -> keep full geometry constraint.
            with torch.no_grad():
                render_residual = (image - gt_image).abs().mean(dim=0)  # [H, W]
                residual_normalized = render_residual / (render_residual.max() + 1e-8)
                # Map: low residual -> 1.0, high residual -> 0.5
                residual_confidence = 1.0 - 0.5 * residual_normalized
                residual_confidence = residual_confidence.clamp(0.5, 1.0)
            # Only activate after initial convergence to avoid premature relaxation
            if iteration <= 5000:
                residual_confidence = torch.ones_like(residual_confidence)
            # ====== End Improvement ======

            if not opt.wo_image_weight:
                normal_loss = weight * (residual_confidence * image_weight * (((depth_normal - normal)).abs().sum(0))).mean()
            else:
                normal_loss = weight * (residual_confidence * (((depth_normal - normal)).abs().sum(0))).mean()
            loss += (normal_loss)
