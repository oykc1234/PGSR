...
def compute_depth_confidence(depth, gt_image, conf_threshold=0.05):
    """
    Generate a per-pixel confidence map for geometry constraints, inspired by confdepth.
    
    Logic:
    - Texture-rich regions: high confidence -> full geometry constraint
    - Weak-texture regions: low confidence -> reduced geometry constraint
    
    This allows the model to relax normal consistency in uncertain areas,
    improving rendering quality without external depth priors.
    
    Args:
        depth: [1, H, W] rendered plane depth
        gt_image: [3, H, W] ground truth image
        conf_threshold: below this value, confidence is set to 0
    
    Returns:
        confidence: [H, W] in [0, 1]
    """
    # 1. Depth spatial gradient (scale-invariant)
    depth_sq = depth.unsqueeze(0) if depth.dim() == 3 else depth.unsqueeze(0).unsqueeze(0)
    depth_pad = F.pad(depth_sq, (1, 1, 1, 1), mode='replicate')
    grad_x = (depth_pad[:, :, 1:-1, 2:] - depth_pad[:, :, 1:-1, :-2]) / 2.0
    grad_y = (depth_pad[:, :, 2:, 1:-1] - depth_pad[:, :, :-2, 1:-1]) / 2.0
    depth_grad = torch.sqrt(grad_x ** 2 + grad_y ** 2 + 1e-8).squeeze()  # [H, W]
    # Normalize by depth value for scale invariance
    depth_grad_rel = depth_grad / (depth.squeeze().abs() + 1e-6)

    # 2. Image texture gradient (Sobel-like)
    ...
    ig_x = (gray_pad[:, :, 1:-1, 2:] - gray_pad[:, :, 1:-1, :-2]) / 2.0
    ig_y = (gray_pad[:, :, 2:, 1:-1] - gray_pad[:, :, :-2, 1:-1]) / 2.0
    img_grad = torch.sqrt(ig_x ** 2 + ig_y ** 2 + 1e-8).squeeze()  # [H, W]

    # 3. Robust normalization using 95th percentile
    depth_conf = (depth_grad_rel / (depth_grad_rel.quantile(0.95) + 1e-8)).clamp(0, 1)
    img_conf = (img_grad / (img_grad.quantile(0.95) + 1e-8)).clamp(0, 1)

    # 4. Combine: geometric mean ensures both signals must agree
    confidence = torch.sqrt(depth_conf * img_conf + 1e-8)

    # 5. Threshold: very low confidence -> 0 (matching confdepth05 threshold)
    confidence[confidence < conf_threshold] = 0.0

    # 6. Light spatial smoothing to avoid noisy weight boundaries
    confidence = F.avg_pool2d(
        confidence.unsqueeze(0).unsqueeze(0), kernel_size=3, stride=1, padding=1
    ).squeeze()

    # 7. Final normalization to [0, 1]
...
def training(dataset, opt, pipe, testing_iterations, saving_iterations, checkpoint_iterations, checkpoint, debug_from):
    ...
    for iteration in range(first_iter, opt.iterations + 1):
        ...
        # single-view loss (depth confidence weighted, inspired by confdepth)
        if iteration > opt.single_view_weight_from_iter:
            ...
            # ====== Depth Confidence Weighting (confdepth-inspired) ======
            with torch.no_grad():
                if iteration > 3000 and 'plane_depth' in render_pkg:
                    depth_confidence = compute_depth_confidence(
                        render_pkg['plane_depth'], gt_image, conf_threshold=0.05
                    )
                    depth_conf_weight = 0.3 + 0.7 * depth_confidence
                else:
                    depth_conf_weight = torch.ones(
                        image_weight.shape, device=image_weight.device
                    )
            # ====== End Depth Confidence Weighting ======

            if not opt.wo_image_weight:
                normal_loss = weight * (depth_conf_weight * image_weight * (((depth_normal - normal)).abs().sum(0))).mean()
            else:
                normal_loss = weight * (depth_conf_weight * (((depth_normal - normal)).abs().sum(0))).mean()
            loss += (normal_loss)
        ...
        if iteration > opt.multi_view_weight_from_iter:
            ...
            if nearest_cam is not None:
                ...
                if iteration % 200 == 0:
                    ...
                    image_weight_vis = image_weight.detach().cpu().numpy()
                    image_weight_vis = (image_weight_vis * 255).clip(0, 255).astype(np.uint8)
                    image_weight_color = cv2.applyColorMap(image_weight_vis, cv2.COLORMAP_JET)
